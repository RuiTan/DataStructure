...
void Family::setOperator() {
    ...
    switch (operate)
    {
        case 'A': {
            ...
            while (Names.find(_name)->second->offspring != nullptr) {
                cout << "该人已经有家庭了，请重新输入：";
                cin >> _name;
            }
            int _offsprings;
        ...
        case 'C': {
            ...
            if (temp->offspring == nullptr) {
                cout << "该人没有子女，无法解散\n";
            }
            else {
                dissolveFamily(temp->offspring);
                temp->offspring = nullptr;
                temp->offsprings = 0;
            }
            cout << head->name;
            printFamily(head);
...
void Family::insertToFamily(string _parent, string _offspring) {
    ...
    if (Names.find(_parent)->second->offspring == nullptr) {
        temp->offspring = member;
        member->parent = temp;
    }
    else {
        temp = temp->offspring;
        while (temp->next_bro != nullptr) {
            temp = temp->next_bro;
        }
...
void Family::dissolveFamily(Member *_name) {
    if (_name->offspring == nullptr && _name->next_bro == nullptr) {
        Names.erase(_name->name);
        delete _name;
        return;
    }
    if (_name->offspring != nullptr)
        dissolveFamily(_name->offspring);
    if (_name->next_bro != nullptr)
...
void Family::printFamily(Member *_member) {
    ...
    cout << _member->name;
    printFamily(_member);
    while (_member->next_bro != nullptr) {
        cout << ",";
